...
根据计算资源切换backbone， **U-Net**不需要可视化交互，对于高精度科研任务/数据充足使用 **Swin Transformer** 或大型 **ViT** 主干网络。
...
1. 交互式平台可接受用户prompt输入来增强分割。如文本prompt（如“寻找球形颗粒”）或视觉prompt（如点击或框选位置）被转化为与图像嵌入维度一致的tokens。
2. 与现有的开源 **Cellpose-SAM** 相比，突出的优点在哪里？
3. 如何做到参数更少？结构化剪枝/低秩矩阵分解/
