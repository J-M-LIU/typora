...
**BN(Batch Normalization):** BN 的目的是在训练过程中稳定每一层的输入分布, 使得每一层的输入都有零均值和单位方差。这是通过对小批量的数据进行归一化, 并引入两个可学习的参数 (尺度和偏移) 来实现的。
$$
y_i=\gamma\left(\frac{x_i-\mu_B}{\sqrt{\sigma_B^2+\epsilon}}\right)+\beta
$$
其中, $\mu_B$ 是批量的均值, $\sigma_B^2$ 是批量的方差, $\gamma$ 和 $\beta$ 是可学习的尺度和偏移参数。 

**GDN(Generalized Divisive Normalization):** GDN 的目的是考虑多个通道之间的相互作 用, 并通过调整 $\beta$ 和 γ 参数来控制这种作用。它是为了图像压缩任务而设计的, 并且考虑了像素之间的相关性。
$$
y[i]=\frac{x[i]}{\sqrt{\beta[i]+\sum_j(\gamma[j,i]*x[j]^2)}}
$$
其中, $\beta$ 和 $\gamma$ 是可学习的参数。

**为什么GDN相比BN是空间自适应的，且具有高度非线性**

- 空间自适应性: 

  - **BN**主要关注小批量中整个通道的统计数据。它将所有空间位置的特征归一化为具有相同的均值和方差。
  - **GDN**的设计考虑了特定的空间位置和多个通道间的相互作用。特别地, GDN的归一化分母考虑了一个特定像素位置的多个通道值。因此, 不同的空间位置可能会有不同的归一化参数, 这使得GDN具有空间自适应性。

- 高度非线性:

  - 在GDN中,归一化的分母是输入特征的二次函数, 它由一个与其他通道的特征相乘的系数矩阵和一个偏置项组成。具体地, GDN的公式为：
    $$
    y[i]=\frac{x[i]}{\sqrt{\beta[i]+\sum_j(\gamma[j,i]*x[j]^2)}}
    $$
    这里, 分母中的项$\sum_j(\gamma[j,i]*x[j]^2)$ 是特征的二次函数。这种二次依赖关系导致了高度的非线性。

- 通道间的相互作用: 
  - GDN的设计考虑了不同通道间的相互作用。特别是, 它的归一化机制由一个系数矩阵 $\gamma$ 控制, 该矩阵决定了一个通道如何受到其他通道的影响。这种跨通道的相互作用是BN所没有的, 它为GDN增添了额外的复杂性和非线性。

## 非线性变换编码框架和VAE的区别

1. **值的域**:
   - VAE是连续值的，而数字压缩（例如，传统的图片编码）在离散域中操作。简单来说，VAE在连续的空间中工作，而传统的压缩技术在离散的空间中工作。
   - 将微分熵与离散熵或与实际的比特率相比较可能导致误导性的结果。这是因为在连续和离散的情境中，“不确定性”的度量是不同的。
2. **优化目标**:
   - 生成模型，如VAE，旨在最小化数据样本的微分熵，这经常意味着尝试减少数据的某种内在变化或“波动”。
   - 与此相反，非线性变换编码方法旨在通过某种“slack”项（这可能是指与数据的某种残差或误差有关的项）来解释数据，这可以最大化或最小化某个参数λ。
3. **压缩模型的性能评估**:
   - 压缩模型的整体性能可以通过考虑所有可能的模型参数值来确定其可达到的失真和比特率的凸包来描述。
   - 找到这个凸包等同于为某些特定的λ值优化模型。
